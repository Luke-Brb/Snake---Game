...
function createFood() {
    let food = {
        x: Math.floor(Math.random() * horizontalSq), // Math.floor - round down the product of Math.random X horizontalSq (eg: 5.9 will become 5)
        y: Math.floor(Math.random() * verticalSq) // Math.random - generate number ​​between 0 and 0.99
    };
    // condition NOT to generate food on the coordinates occupied by the snake
...
function setDirection(e) {
  ...
  if ((newDirection === directions.LEFT && oldDirection !== directions.RIGHT) || (newDirection === directions.RIGHT && oldDirection !== directions.LEFT) || (newDirection === directions.UP && oldDirection !== directions.DOWN) || (newDirection === directions.DOWN && oldDirection !== directions.UP)) {
...
function moveSnake() {
  ...
  if (directionsQueue.length) {
      ...
      // Change the direction of the snake only if
      // the "directionQueue" string that saves the direction changes is not empty
  }
  // Positioning new direction for snake head
...
function hitWall() {
  const head = snake[0];
  return (head.x < 0 || head.x >= horizontalSq || head.y < 0 || head.y >= verticalSq);
}
